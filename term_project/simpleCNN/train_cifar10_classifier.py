...
class CNN(object):
	def __init__(self):
		self.layer1 = Conv2d(3,16,kernel_size = (5,5), stride = (1,1), padding = (2,2),bias = True)
		self.layer2 = Maxpool2d(kernel_size = (2,2))
		self.layer3 = Conv2d(16,36,kernel_size = (5,5), stride = (1,1), padding = (2,2),bias = True)
		self.layer4 = Maxpool2d(kernel_size = (2,2))
		self.layer5 = Linear(2304,128)
		self.layer6 = Linear(128,10)
	def forward(self,x):
		x = self.layer1.forward(x)
		x = self.layer2.forward(x)
		x = self.layer3.forward(x)
		x = self.layer4.forward(x).reshape(-1,2304)
	...
	def backprop(self,delta_loss, lr = 0.1):
		...
		delta_loss = np.asarray(self.layer5.backprop(delta_loss,lr = lr)).reshape(-1,36,8,8)
